...
*/
	class Tools extends inventory{
	 ...
	 function editTool($primarykey,$InventoryName,$Quantity,$supplierID,$InventoryCategory){
		$result = $this->editInventory($primarykey,$InventoryName,$Quantity,$supplierID,' ',$InventoryCategory);
		   if($result)
		  {
			  return true;
		  }else{
		...
		*/
		function getTools($filter=false){
			$strQuery = "Select toolId, toolName, quantity, suppliers.supplierName ,supplierId from tools, suppliers where suppliers.suppliersId = tools.supplierId ";
			if($filter!=false){
